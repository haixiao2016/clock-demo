...
Page({
  ...
  getPrev(){
    ...
    if (userCalendar.length > 0) {
      ...
      this.data.prev_times = prev.calendarAf == undefined ? prev.calendarM0 : prev.calendarAf
  ...
  calendar(){
    ...
    if (_date.getHours()<12){
      // 小于十二小时为上午签到
      // 判断今天是否已经签到==>已签到，退出
      if (userCalendar.length > 0 && userCalendar[userCalendar.length - 1].date == nowTimes){
        // 已签到
        wx.showToast({
          title: '请勿重复签到',
          icon: 'none',
      ...
      if (userCalendar.length > 0 && userCalendar[userCalendar.length - 1].date == nowTimes){
        userCalendar[userCalendar.length - 1].calendarAf = utils.formatTime(_date)
      }else{
        userCalendar.push({
          date: nowTimes,
          week: date_arr[_date.getDay()],
